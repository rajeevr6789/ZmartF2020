...
class Smart_Controller extends CI_Controller {
	...
	*/
	...
	// private $module,$moduleID,$tablestudent,$tableteacher,$tableschool,$endrolled,$tableattend,$tableactivity,$tableslots;
	 private $module,$moduleID,$tablestudent,$tableteacher,$tableschool,$endrolled,$tableattend,$tableactivity,$tableslots,$module_name,$logged_in,$tableclass,$brn_ass,$pass=1234;
	...
	*/
	...
	if($account==1) {
		$loginArr['login']					= $this->admin_model->loginProcess($username,$enpassword);
	
	if($loginArr['login']['status']==1) {
		$this->session->set_userdata($loginArr['login']);
		$this->session->set_userdata("status",$loginArr['login']['status']);
		$this->session->set_userdata("user_id",$loginArr['login']['user_id']);
		$this->session->set_userdata("type",$loginArr['login']['type']);
		$this->session->set_userdata("ses_username",$loginArr['login']['username']);
		$schoolID							= $loginArr['login']['school'];
		
		if($loginArr['login']['type']==1){
			redirect('admin');
		}else{$this->session->set_flashdata('err', 'Please Select Branch Administrator!');
		redirect('admin/login');}
	}else{$this->session->set_flashdata('err', 'You are not admin!');
		redirect('admin/login');}
	...
	}else if($account==2)
	 {
		if($username == 'admin')
		{
			$this->session->set_flashdata('err', 'You are not branch administrator!');
			redirect('admin/login');	
		}
		$loginArr['login']					= $this->admin_model->loginProcess($username,$enpassword);
		...
		$this->session->set_userdata("ses_username",$loginArr['login']['username']);
		//$schoolID							= $loginArr['login']['school'];
			if($loginArr['login']['type']==2) { 				
			
				$branch				= 	$this->input->post('branch');
				//echo $branch;die();
				if(!$branch)
				{
				 $this->session->set_flashdata('err', 'Please Select Branch');
				 redirect('admin/login');	
				}else{
					
					$user_id			= $this->admin_model->loginProcess_branch($username,$enpassword);
					//echo $user_id['user_id'];die();
                    $this->session->set_userdata("br_usr_id",$user_id['user_id']);
					
					$get_brh_id			=	$this->admin_model->get_brh_id($branch);
									print_r($get_brh_id);	
					//echo $get_brh_id['brh_name'];die();
					$check_branch		=	$this->admin_model->check_branch($user_id['user_id'],$get_brh_id['brh_id']);			 
					//print_r($check_branch);die();
					if(!empty($check_branch)){
						
						$this->session->set_userdata("module_name",$branch); 
						$this->session->set_userdata("br_name",$get_brh_id['brh_name']);                
						redirect($branch.'/admin');
					}else{
						$this->session->set_flashdata('err', 'Wrong branch selection');
						redirect('admin/login');
					}
				}		
			  
	}} else { 
		$this->session->set_flashdata('err', 'Incorrect username or password!');
		redirect('admin/login');
	}
    }else if($account==3){ //teacher login
	...
	$branch				= 	$this->input->post('branch');
		...
		$schname['get_module'] 			= $this->admin_model->school_module($username);
		$schname['get_module_branch'] 	= $this->admin_model->school_module_branch($username,$branch);
		
	//echo "<pre>";
	//print_r($schname['get_module']);
	//echo $branch;die();
	if($schname['get_module']['status']==false)
	{
		$this->session->set_flashdata('err', 'Incorrect username or password!');
		redirect('admin/login');
	}
	if($schname['get_module_branch']['status_branch']==false)
	{
		$this->session->set_flashdata('err', 'Wrong Branch Selection!');
		redirect('admin/login');
	}
	
	$module_name	=	$schname['get_module']['module_name'];	
	$teacher_table	=	$module_name.'_teacher';
	$slot_table		=	$module_name.'_slots';
	$teacher['get_id']		= $this->admin_model->get_id($username,$teacher_table);
	$teacher_id		=	$teacher['get_id']['id'];
	$chk_id_slot['id_chk']	= $this->admin_model->chk_teacher_id($teacher_id,$slot_table);
	
	if($chk_id_slot['id_chk']['status'] == true)
	{
		$teacher_class		=	$chk_id_slot['id_chk']['class'];	
		$loginArr['login']		= $this->admin_model->teacherLogin($username,$password);
		...
		if($loginArr['login']['status']==1) {
			...
			$this->session->set_userdata("user_id",$loginArr['login']['user_id']);
			$this->session->set_userdata("type",$loginArr['login']['type']);
			$modulename							= $loginArr['login']['school'];
			$teacherDet		= $this->admin_model->teacherByemail($modulename.'_teacher',$loginArr['login']['user_id']);
			$this->session->set_userdata("ses_username",$teacherDet['tea_name']);
			$branchname 	= $this->admin_model->school_branchname_bymodulename($schname['get_module_branch']['module_name']);
			$this->session->set_userdata("br_name",$branchname);
			redirect($modulename.'/teacher');
			} else { 
			$this->session->set_flashdata('err', 'Incorrect username or password!');
			redirect('admin/login');
			}
	}else{
		$this->session->set_flashdata('err', 'Cannot acces, no class assigned to this teacher');
	    redirect('admin/login');
		}
	}
